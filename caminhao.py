...
class Caminhao(V_Quatro_Rodas,Veiculo_Motorizado):
    ...
    def adicionar_carga(self,qtd_carga):
        if qtd_carga+ self._carga> self._capacidade_carga:
            print("Operação negada. A carga maxima do Caminhao seria ultrapassada")
        elif qtd_carga + self._carga == self._capacidade_carga:
            self._carga += qtd_carga
            print("A carga máxima foi atingida")
        else: 
            print(f"Caminhao abastecida com carga {qtd_carga}. Agora a Caminhao possui {self._carga} kg de carga")